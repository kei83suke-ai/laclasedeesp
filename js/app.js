...
function formatSpanishAudioText(text) {
  ...
  const expanded = tokens.map(token => {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      // 冠詞はそのまま
      if (['el','la','los','las','un','una','unos','unas'].includes(suf.toLowerCase())) {
        variants.push(suf);
        continue;
      }

      // 語尾変化パターン（1〜2文字の接尾辞）
      let stemTrimmed = false;
      let stem = first;
      if (suf === 'a' && first.endsWith('o'))       { stem = first.slice(0, -1); stemTrimmed = true; }
      else if (suf === 'as' && first.endsWith('os')) { stem = first.slice(0, -2); stemTrimmed = true; }
      else if (suf === 'as' && first.endsWith('o'))  { stem = first.slice(0, -1); stemTrimmed = true; }
      else if (suf === 'os' && first.endsWith('o'))  { stem = first.slice(0, -1); stemTrimmed = true; }
      else if (suf === 'e' && first.endsWith('o'))   { stem = first.slice(0, -1); stemTrimmed = true; }

      if (stemTrimmed) {
        // 接尾辞を語幹に結合: Alto/a → Alta
        variants.push(stem + suf);
      } else if (suf.length > 1) {
        // 完全な単語（例: Barato/Barata）→ そのまま使う
        variants.push(suf);
      } else {
        // 1文字の接尾辞でステム変化なし
        variants.push(first + suf);
      }
    }
    return variants;
  });
  ...
  for (let i = 0; i < maxVariants; i++) {
    phrases.push(expanded.map(v => v[Math.min(i, v.length - 1)]).join(' '));
  }
